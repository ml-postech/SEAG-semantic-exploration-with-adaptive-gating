...
class Llama3Model(LanguageModel):
    ...
    def __init__(self, path, size, max_batch_size=1, max_seq_len=2048, **kwargs):
        ...
        self.model, self.tokenizer = self.build(
            os.path.join(path, f"Meta-Llama-3-{size}"),
            os.path.join(path, f"Meta-Llama-3-{size}", "tokenizer.model"),
            max_seq_len=max_seq_len,
            max_batch_size=max_batch_size,
            **kwargs)
    ...
    def generate(self,
        ...
        input_tokens_count = sum(len(t) for t in prompt_tokens)
        ...
        return GenerateOutput(out_tokens, out_logprobs), input_tokens_count, out_tokens_count
    ...
    def get_next_token_logits(self, prompt: Union[str, list[str]], candidates: Union[list[str], list[list[str]]], **kwargs) -> list[np.ndarray]:
        ...
        input_tokens_count = sum(len(t) for t in prompts_tokens)
        ...
        return logits, input_tokens_count
