...
class Evaluator():
    ...
    def evaluate(self,
        ...
        print('_dataset_name:', self._dataset_name)
        # for 400 cases of GSM8K
        if 'gsm8k' in self._dataset_name:
        ...
        for i, example in enumerate(tqdm(self.dataset,
            ...
            try:
                ...
                algo_output, llm_usage_stats_dict = reasoner(self.input_processor(example),
                                        prompt=self.sample_prompt(
                                            shuffle_prompt=shuffle_prompt,
                                            num_shot=num_shot))
                num_inf_for_get_actions, num_inf_for_answer, num_inf_for_fast_reward = llm_usage_stats_dict['num_inf_for_get_actions'], llm_usage_stats_dict['num_inf_for_answer'], llm_usage_stats_dict['num_inf_for_fast_reward']
                total_num_inf = num_inf_for_get_actions + num_inf_for_answer + num_inf_for_fast_reward

                input_tokens_count_for_get_actions, output_tokens_count_for_get_actions = llm_usage_stats_dict['input_tokens_count_for_get_actions'], llm_usage_stats_dict['output_tokens_count_for_get_actions']
                input_tokens_count_for_answer, output_tokens_count_for_answer = llm_usage_stats_dict['input_tokens_count_for_answer'], llm_usage_stats_dict['output_tokens_count_for_answer']
                input_tokens_count_for_fast_reward, output_tokens_count_for_fast_reward = llm_usage_stats_dict['input_tokens_count_for_fast_reward'], llm_usage_stats_dict['output_tokens_count_for_fast_reward']
                total_tokens_count = input_tokens_count_for_answer + output_tokens_count_for_answer + input_tokens_count_for_get_actions + output_tokens_count_for_get_actions + input_tokens_count_for_fast_reward + output_tokens_count_for_fast_reward
                ...
                log_str = f'Case #{resume + i + 1}: {correct=}, {output=}, {answer=};'\
                            ...
                            f'{num_inf_for_get_actions=}, {num_inf_for_answer=}, {num_inf_for_fast_reward=}, {total_num_inf=}, {input_tokens_count_for_get_actions=}, {output_tokens_count_for_get_actions=}, {input_tokens_count_for_answer=}, {output_tokens_count_for_answer=}, {input_tokens_count_for_fast_reward=}, {output_tokens_count_for_fast_reward=}, {total_tokens_count=}'
                tqdm.write(log_str)
    ...
    def evaluate_sc(self,
        ...
        for i, example in enumerate(tqdm(self.dataset,
            ...
            prompt = self.sample_prompt(
                            shuffle_prompt=shuffle_prompt,
                            num_shot=num_shot)
            output_list, save_list = list(), list()
            for j in range(n_sc):
